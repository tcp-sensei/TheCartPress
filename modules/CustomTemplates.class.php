...
<p><?php _e( 'No templates', 'tcp' );?></p>
	...
	function save( $post_id, $post ) {
		if ( !wp_verify_nonce( isset( $_POST['tcp_ct_noncename'] ) ? $_POST['tcp_ct_noncename'] : '', 'tcp_ct_noncename' ) ) {
	...
	function delete( $post_id ) {
		...
		if ( !wp_verify_nonce( isset( $_POST['tcp_noncename'] ) ? $_POST['tcp_noncename'] : '', 'tcp_ct_noncename' ) ) {
			return $post_id;
		}
		if ( !current_user_can( 'edit_post', $post_id ) ) {
			return $post_id;
