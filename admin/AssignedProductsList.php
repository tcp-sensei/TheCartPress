		...
		foreach( $assigned_list as $assigned ) :
			...
			<td class="tcp_meta_value">
				<form method="post" name="frm_delete_relation_<?php echo $assigned->id_to; ?>" id="frm_create_relation_<?php echo $assigned_post->id_to; ?>">
					...
					&nbsp;|&nbsp;
					<label for="list_order"><?php echo _x( 'Order', 'to sort the list', 'tcp' ); ?>:&nbsp;</label><input type="text" min="0" name="list_order" id="list_order" size="2" maxlength="4" value="<?php echo $assigned->list_order; ?>" class="tcp_count"/>
					<label for="units"><?php _e( 'Units', 'tcp' ); ?>:&nbsp;</label><input type="text" min="0" name="units" id="units" size="2" maxlength="4" value="<?php echo $units; ?>" class="tcp_count"/>
	...
	if ( ( tcp_is_saleable_post_type( $post_type_to ) && $category_slug && $product_type ) || ( $post_type_to == 'post' && $category_slug ) ) :
		...
		if ( is_array( $posts ) && count( $posts ) > 0 ) :
			foreach( $posts as $id ) : $post = get_post( $id ); ?>
				...
				else echo $post->post_excerpt; ?></td>
				<td class="tcp_meta_value">
				<div class="wrap">
					<form method="post" name="frm_create_relation_<?php echo $post->ID; ?>" id="frm_create_relation_<?php echo $post->ID; ?>">
						...
						<input id="product_type" name="product_type" value="<?php echo $product_type; ?>" type="hidden" />
						<input id="category_slug" name="category_slug" value="<?php echo $category_slug; ?>" type="hidden" />
						| <label for="list_order"><?php _e( 'Order', 'tcp' ); ?>:&nbsp;</label><input type="text" name="list_order" id="list_order" size="2" maxlength="4" value="0" class="tcp_count"/>
						<label for="units"><?php _e( 'units', 'tcp' ); ?>:&nbsp;</label><input id="units" name="units" value="1" size="2" maxlength="3" type="text" class="tcp_count"/>
